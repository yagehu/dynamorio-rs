...
impl Closure {
    fn lookup_code(args: usize) -> &'static [u8] {
        match args {
            ...
            6 => &[
                0xff, 0x35, 0xea, 0xff, 0xff, 0xff, 0xff, 0x15, 0xec, 0xff, 0xff, 0xff, 0x48, 0x83,
                0xc4, 0x08, 0xc3,
            ],
            7 => &[
                0x48, 0x83, 0xec, 0x08, 0xff, 0x35, 0xe6, 0xff, 0xff, 0xff, 0xff, 0x74, 0x24, 0x18,
                0xff, 0x15, 0xe4, 0xff, 0xff, 0xff, 0x48, 0x83, 0xc4, 0x18, 0xc3,
            ],
            8 => &[
                0xff, 0x35, 0xea, 0xff, 0xff, 0xff, 0xff, 0x74, 0x24, 0x18, 0xff, 0x74, 0x24, 0x18,
                0xff, 0x15, 0xe4, 0xff, 0xff, 0xff, 0x48, 0x83, 0xc4, 0x18, 0xc3,
            ],
    ...
    pub fn new(args: usize, callback: fn(), user_data: *mut core::ffi::c_void) -> Self {
        let code = Self::lookup_code(args);
        let size = 2 * core::mem::size_of::<usize>() + code.len();

        let inner =
            unsafe { dr_nonheap_alloc(size, DR_MEMPROT_READ | DR_MEMPROT_WRITE | DR_MEMPROT_EXEC) };

        let storage: &mut [usize] =
            unsafe { core::slice::from_raw_parts_mut(inner as *mut usize, 2) };
    ...
    pub fn code(&self) -> *mut core::ffi::c_void {
        unsafe {
            (self.inner as *mut u8).add(2 * core::mem::size_of::<u64>()) as *mut core::ffi::c_void
        }
    }
}
