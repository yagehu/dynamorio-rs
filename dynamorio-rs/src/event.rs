...
unsafe extern "C" fn exit_wrapper<T: ExitHandler>(handler: &Mutex<T>) {
...
impl<T: ExitHandler> Drop for RegisteredExitHandler<T> {
    fn drop(&mut self) {
        let func: extern "C" fn() = unsafe { core::mem::transmute(self.closure.code()) };
...
pub fn register_exit_handler<T: ExitHandler>(handler: &Arc<Mutex<T>>) -> RegisteredExitHandler<T> {
    let closure = Closure::new(
        0,
        unsafe {
            core::mem::transmute::<unsafe extern "C" fn(&Mutex<T>), fn()>(exit_wrapper::<T>)
            // as unsafe extern "C" fn(_))
        },
        Arc::as_ptr(handler) as *mut core::ffi::c_void,
    );

    let func: extern "C" fn() = unsafe { core::mem::transmute(closure.code()) };
...
pub fn register_nudge_event(func: fn(&mut Context, u64) -> (), client_id: ClientId) {
