...
impl ModuleData {
    ...
    pub fn main_module() -> Self {
        let raw = unsafe { dr_get_main_module() };
    ...
    pub fn from_address(address: usize) -> Option<Self> {
        let raw = unsafe { dr_lookup_module(address as _) };
    ...
    pub fn start(&self) -> usize {
        unsafe { (*self.raw).__bindgen_anon_1.start as usize }
    }

    pub fn end(&self) -> usize {
        unsafe { (*self.raw).end as usize }
    }
    ...
    pub fn preferred_name(&self) -> Option<&str> {
        let s = unsafe { dr_module_preferred_name(self.raw) };
    ...
    pub fn contains(&self, address: usize) -> bool {
        unsafe { dr_module_contains_addr(self.raw, address as app_pc) != 0 }
    }

    pub fn get_proc_address(&self, name: &str) -> Option<unsafe extern "C" fn()> {
        let name = CString::new(name).unwrap();

        unsafe { dr_get_proc_address((*self.raw).__bindgen_anon_1.handle, name.as_ptr()) }
    }
    ...
    pub fn instrumented(&self) -> bool {
        unsafe { dr_module_should_instrument((*self.raw).__bindgen_anon_1.handle) != 0 }
    }
...
impl Clone for ModuleData {
    fn clone(&self) -> Self {
        let raw = unsafe { dr_copy_module_data(self.raw) };
